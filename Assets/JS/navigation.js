(() => {
  ...
  headers.forEach((navList, index) => {
    ...
    Array.from(header.children)
      .filter(
        (item) => item !== navList && item.classList?.contains("dropdown"),
      )
      .forEach((dropdown) => {
    ...
    const socialDropdown = Array.from(
      navList.querySelectorAll(".dropdown"),
    ).find(
      (dropdown) =>
        dropdown.querySelector(".dropdown-toggle")?.textContent.trim() ===
        "Socials",
    );

    const socialMenu = socialDropdown?.querySelector(".dropdown-menu");
    if (socialMenu) {
      socialMenu.replaceChildren(
        ...["X - coming soon", "Instagram - coming soon"].map((label) => {
